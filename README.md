...
| 方式 | Blender 可见 | MMD 可见 | 导出保留 | 原理 |
|------|-------------|---------|---------|------|
| **简单物理（推荐）** | ✅ | ✅ | ✅ | type=1 Dynamic + ±10° 关节，参考 Purifier Inase 18 |
| **弹簧模拟** | ✅ | ❌ 需重新运行 | ❌ | 数学模拟 → 骨骼关键帧 |
| **RGBA 5刚体** | ❌ | ✅ | ✅ | 零限制技巧（仅 MMD Bullet 有效） |

详细操作请参考 [用户手册](docs/user_manual.md)。

## 注意事项

- Blender Bullet 与 PMXEditor Bullet 行为不同，参数不可直接沿用
- 本插件不会删除或修改非自身创建的刚体/关节（除非勾选 Overwrite）
- 数学弹簧模拟方案不依赖 Blender 物理系统，兼容性最好
